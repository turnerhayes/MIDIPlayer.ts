enum GeneratorTypes {
  ...
  // the last sample data point to be played for this instrument. For example,
  // if End were 17 and endAddrOffset were -2, the last sample data point played
  ...
  // the sample data point considered equivalent to the Startloop sample data point
  // for the loop for this instrument. For example, if Endloop were 15 and endloopAddrsOffset
  ...
  // Envelope will influence pitch. A positive value indicates an increase in pitch;
  // a negative value indicates a decrease in pitch. Pitch is always modified
  ...
  // frequency exceeds 20 kHz and the Q(resonance) of the filter is zero, the filter does
  // not affect the signal.
  "initialFilterFc" = 8,
  // This is the height above DC gain in centibels which the filter resonance exhibits at
  // the cutoff frequency. A value of zero or less indicates the filter is not resonant;
  // the gain at the cutoff frequency (pole angle) may be less than zero when zero is
  ...
  // longer than one second. The most negative number (-32768) conventionally indicates
  // no delay. For example, a delay of 10 msec would be 1200 log2(.01) = -7973.
  "delayModLFO" = 21,
  ...
  // For example, a frequency of 10 mHz would be 1200 log2(.01 / 8.176) = -11610.
  "freqModLFO" = 22,
  ...
  // SoundFont 2.01 Technical Specification - Page 35 - Printed 12/10/1996 5:57 PM
